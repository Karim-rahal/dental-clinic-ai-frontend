...
export default function PatientDashboard() {
  ...
  const savePhoto = (url: string) => {
    setPhotoUrl(url);
    localStorage.setItem("patient_photo", url);
  };
  ...
  const removePhoto = () => {
    setPhotoUrl(null);
    localStorage.removeItem("patient_photo");
    closePhotoModal();
  };
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      case "scheduled": return { bg: "rgba(62,180,137,0.1)", color: COLORS.green, border: `1px solid ${COLORS.mint}` };
...
/>
              ...
              {cameraError && (
                <div style={{ padding: "10px 14px", backgroundColor: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 10, color: "#DC2626", fontSize: 12, marginBottom: 16 }}>{cameraError}</div>
              )}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  {photoUrl && (
                    <button onClick={removePhoto}
                      style={{ padding: "11px", backgroundColor: "transparent", color: "#e05555", border: "1px solid rgba(224,85,85,0.3)", borderRadius: 12, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", letterSpacing: "0.08em" }}
                    >Remove Photo</button>
                  )}
                </div>
              )}
  ...
  const navItems = [
    ...
    { id: "appointments", label: "Appointments", icon: "M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01", path: "/patient/appointments" },
    { id: "book", label: "Book Visit", icon: "M12 5v14M5 12h14", path: "/patient/book" },
    { id: "upload", label: "Upload X-Rays", icon: "M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M17 8l-5-5-5 5M12 3v12", path: "/patient/upload" },
    { id: "profile", label: "Profile", icon: "M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2M12 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8z", path: "/patient/profile" },
  ];
    ...
    <main style={{ minHeight: "100vh", backgroundColor: "#F4F7FA", fontFamily: "'Josefin Sans', sans-serif", display: "flex" }}>
      ...
      <div style={{ width: 220, backgroundColor: COLORS.white, borderRight: "1px solid rgba(167,228,216,0.3)", display: "flex", flexDirection: "column", position: "fixed", top: 0, bottom: 0, left: 0, zIndex: 50, boxShadow: "2px 0 20px rgba(44,62,80,0.04)" }}>
        ...
        <div style={{ padding: "20px 12px", flex: 1, overflowY: "auto" }}>
          <div style={{ fontSize: 9, fontWeight: 700, color: COLORS.navyMid, letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: 8, paddingLeft: 8 }}>General</div>
          {navItems.map((item) => (
            <button key={item.id}
              onClick={() => { setActiveNav(item.id); if (item.path) router.push(item.path); }}
              style={{
                width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "10px 12px",
                borderRadius: 10, border: "none", cursor: "pointer", fontFamily: "inherit",
                backgroundColor: activeNav === item.id ? COLORS.lightMint : "transparent",
                color: activeNav === item.id ? COLORS.green : COLORS.navyMid,
        ...
        <div style={{ padding: "16px", borderTop: "1px solid rgba(167,228,216,0.3)" }}>
          <div style={{ backgroundColor: COLORS.navy, borderRadius: 16, padding: "16px", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ position: "relative", zIndex: 1 }}>
              <div style={{ fontSize: 9, color: "rgba(167,228,216,0.7)", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 8 }}>Book online.</div>
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", marginBottom: 12, lineHeight: 1.5 }}>No queues. Hassle-free.</div>
      ...
      <div style={{ marginLeft: 220, flex: 1, display: "flex", minHeight: "100vh" }}>
        ...
        <div style={{ flex: 1, padding: "28px 24px", overflowY: "auto" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
            <div>
              <div style={{ fontSize: 11, color: COLORS.navyMid, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 4 }}>Patient Portal</div>
...
>
  ...
  />
          ...
          >
            {loading ? (
              <div style={{ height: 100, display: "flex", alignItems: "center", justifyContent: "center", color: COLORS.navyMid, fontSize: 13 }}>Loading...</div>
            ) : (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 20 }}>
            {[
              ...
              { label: "Upcoming", value: loading ? "—" : upcoming.length, color: COLORS.green, bg: "rgba(62,180,137,0.1)", icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={COLORS.green} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2C8 2 5 5 5 8c0 2 .5 3.5 1 5l1 8c.2 1 1 1 1.5 0L9 17h6l.5 4c.5 1 1.3 1 1.5 0l1-8c.5-1.5 1-3 1-5 0-3-3-6-7-6z"/></svg> },
              { label: "Pending", value: loading ? "—" : appointments.filter(a => a.status === "pending").length, color: "#d97706", bg: "rgba(245,158,11,0.1)", icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#d97706" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> },
            ].map((stat, i) => (
              ...
              >
                <div style={{ width: 44, height: 44, borderRadius: 14, backgroundColor: stat.bg, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16 }}>
                  {stat.icon}
                </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <div style={{ fontSize: 14, fontWeight: 900, color: COLORS.navy }}>Notes</div>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 12 }}>
                {notes.map((note, i) => (
                  <div key={i} onClick={() => setCheckedNotes(prev => prev.includes(i) ? prev.filter(x => x !== i) : [...prev, i])}
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[
                  { label: "Book Appointment", path: "/patient/book", color: COLORS.green },
                  { label: "View All Appointments", path: "/patient/appointments", color: COLORS.navy },
                  { label: "Upload X-Rays", path: "/patient/upload", color: "#6366f1" },
                  { label: "Call AI Receptionist", href: "tel:+15186346766", color: "#f59e0b" },
                ].map((action) => (
                  <button key={action.label}
                    onClick={() => action.href ? window.location.href = action.href : router.push(action.path!)}
...
<div style={{ marginTop: 16 }}>
  ...
  {selectedDate && filteredAppointments.length === 0 && (
    <div style={{ fontSize: 12, color: COLORS.navyMid }}>
  ...
  {filteredAppointments.map((apt) => {
    ...
    return (
      ...
      >
        ...
        <div style={{ fontSize: 11, color: COLORS.navyMid }}>
          {time}
        </div>
      </div>
    );
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
            ...
            {loading ? (
              <div style={{ fontSize: 12, color: COLORS.navyMid, padding: "12px 0" }}>Loading...</div>
            ) : upcoming.length === 0 ? (
              <div style={{ fontSize: 12, color: COLORS.navyMid, padding: "12px 0" }}>No upcoming appointments</div>
            ) : (
                ...
                return (
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                      <div style={{ width: 32, height: 32, borderRadius: 8, backgroundColor: COLORS.green, display: "flex", alignItems: "center", justifyContent: "center", color: "white", flexShrink: 0 }}>
                        <ToothIcon size={16} />
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 12, fontWeight: 700, color: COLORS.navy }}>{apt.service}</div>
                        <div style={{ fontSize: 10, color: COLORS.navyMid }}>Bright Smile Clinic</div>
                      </div>
                    </div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} style={{ marginTop: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div style={{ fontSize: 14, fontWeight: 900, color: COLORS.navy }}>Your Treatment</div>
              <button onClick={() => router.push("/patient/appointments")} style={{ fontSize: 10, color: COLORS.green, background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", fontWeight: 700, letterSpacing: "0.08em" }}>See more</button>
            </div>
            <div style={{ fontSize: 11, color: COLORS.navyMid, marginBottom: 12 }}>All your recent appointments</div>
            {loading ? (
              ...
              appointments.slice(0, 4).map((apt) => {
                ...
                return (
                  <div key={apt.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid rgba(167,228,216,0.3)" }}>
                    <div style={{ width: 32, height: 32, borderRadius: 8, backgroundColor: COLORS.lightMint, border: `1px solid ${COLORS.mint}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <ToothIcon size={14} />
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12, fontWeight: 700, color: COLORS.navy, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{apt.service}</div>
                      <div style={{ fontSize: 10, color: COLORS.navyMid }}>{date}</div>
                    </div>
          ...
          >
            ...
            <div style={{ position: "relative", zIndex: 1, display: "flex", gap: 12, alignItems: "center" }}>
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 900, color: "white" }}>{user?.full_name}</div>
